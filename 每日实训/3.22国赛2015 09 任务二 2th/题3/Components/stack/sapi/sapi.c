    ...
    while ( pMsg )
    {
      switch ( pMsg->event )
      {
        ...
        case AF_INCOMING_MSG_CMD:
          ...
          /* user code start */
          uint8 buf[2];
          osal_memcpy(buf,pMSGpkt->cmd.Data,2);
          if(buf[0] == 'R' && buf[1] == '1')
          {
            set_relay(1);
            P1_3 = 1;
            if(++flag==2)
            {
              uint8 uart[12] = {0XFE,0X01,0X00,0X01,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0XFF};
              HalUARTWrite(HAL_UART_PORT_0, uart, 12);
              flag = 0;
            }
          }
          else if(buf[0] == 'R' && buf[1] == '0')
          {
            ...
            if(++flag==2)
            {
              uint8 uart[12] = {0XFE,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0XFF};
              HalUARTWrite(HAL_UART_PORT_0, uart, 12);
              flag = 0;
            }
          }
