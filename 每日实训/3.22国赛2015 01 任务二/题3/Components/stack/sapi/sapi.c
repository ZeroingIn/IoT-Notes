...
#include "sensor.h" //引用继电器声明

extern uint8 zgStartDelay;
extern uint8 zgSapiEndpoint;
extern void ChannelPanidInit (void);
/*********************************************************************
 * CONSTANTS
 */
 ...
 * @return  none
 */
unsigned char flag = 0; //1为自动控制
unsigned char flag1 = 0; //接收消息次数
uint8 hhhh[12] = {0XFE,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0XFF}; //全局数组用来存储发送串口的数据
UINT16 SAPI_ProcessEvent( byte task_id, UINT16 events )
{
  ...
  uint8 buf[16];
  if ( events & SYS_EVENT_MSG )//系统任务事件
  {
    ...
    while ( pMsg )
    {
      switch ( pMsg->event )
      {
        ...
        case AF_INCOMING_MSG_CMD:
          ...
          /* user code start */
          uint8 inf[2];
          osal_memcpy(inf,pMSGpkt->cmd.Data,2);
          if(flag == 0 && inf[0] == '1') //flag为0时是自动控制
          {
            if(inf[1] < 0x09)
              P1_3 = 1;
            else
             ...
             hhhh[1] = inf[1]; //当接收到了光电发送的数据则将AD值赋值给数组
          
          if(inf[0] == 'L')
              hhhh[3] = inf[1];  //将接收到的LED状态赋值给数组
          
          if(++flag1 == 2)  //接收到两次消息则发送数据至串口
          {
            HalUARTWrite(HAL_UART_PORT_0, hhhh, 12);
            flag1 = 0;
          }
          /* user code end */
          break;
  ...
  if ( events & ( ZB_TIME_EVENT ) )
  {
    ...
    test_read_flash();
      ...
      if(!rdcfg)
      {
        ...
        MicroWait (1000);
        uart_printf("\r\n新大陆物联网\r\n");
        uart_printf("Panid: 0X%04x\r\n", BUILD_UINT16(buf[0],buf[1]));
        uart_printf("chanel: %d\r\n", buf[2]);
        ChannelPanidInit ();
      }
...
void zb_SendZigbeeDatas (uint8 *pData, uint8 lens)
{
  zb_SendDataRequest( 0xFFFF, DUMMY_REPORT_CMD_ID , lens,pData, 0, AF_TX_OPTIONS_NONE, 0 );
}