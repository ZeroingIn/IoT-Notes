...
static void sendDummyReport(void)
{
  ...
  status[1] = '1';
  status[2] = '0';  //默认为0
  status[2] = get_rentihongwai() == 0x01?'1':'0';  //1 have 0 no
  zb_SendZigbeeDatas(status,3); //datas[] lenth
  /* user code end*/
}
...
{
  /* user code start */
  uint8 channel = zb_Readchannel();
  uint8 panid[2];
  zb_Readpandid(panid);
  if(channel != 11 || panid[0] != 0x06 || panid[1] != 0x80)
  {
    panid[0] = 0x06;
    panid[1] = 0x80;
    channel = 11;
