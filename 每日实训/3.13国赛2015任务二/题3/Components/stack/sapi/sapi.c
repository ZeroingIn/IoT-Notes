    ...
    while ( pMsg )
    {
      switch ( pMsg->event )
      {
        ...
        case AF_INCOMING_MSG_CMD:
          ...
          osal_memcpy(buffer,pMSGpkt->cmd.Data,3);
          uint8 datas[12] = {0xFE,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xFF};
          if(buffer[0] == 'E' && buffer[1] == '1' && buffer[2] == '1'){
            //uart_printf("\r\n有人 打开风扇\r\n");
            set_relay(1); //开继电器
            datas[1] = 0x01;
            datas[2] = 0x00;
            datas[3] = 0x01;
            datas[4] = 0x00;
            HalUARTWrite(HAL_UART_PORT_0,datas,12); //按协议发送数据至串口
          }
          else if(buffer[0] == 'E' && buffer[1] == '1' && buffer[2] == '0')
          {
            //uart_printf("\r\n没人 关闭风扇\r\n");
            set_relay(0); //关继电器
            datas[1] = 0x00;
            ...
            datas[3] = 0x00;
            datas[4] = 0x00;
            HalUARTWrite(HAL_UART_PORT_0,datas,12);
          }
          /* user code end */
          break;
