...
void SAPI_ReceiveDataIndication( uint16 source, uint16 command, uint16 len, uint8 *pData  )
{
  ...
  if ( SAPICB_CHECK( SPI_CB_SAPI_RCV_DATA_IND ) )
  {
    ...
    #endif  //MT_SAPI_CB_FUNC
  {
    uart_printf("\r\ncoord1:%x,%x",pData[0],pData[1]);
    pData[0] = 0x55;
    pData[1] = 0xaa;
    zb_SendDataRequest( 0xFFFF, DUMMY_REPORT_CMD_ID , 2,pData, 0, AF_TX_OPTIONS_NONE, 0 );
  }
#endif  //(LOG_TYPE==0) 
#if (LOG_TYPE==2)
  uart_printf("\r\nenddev1:%x,%x",pData[0],pData[1]);
