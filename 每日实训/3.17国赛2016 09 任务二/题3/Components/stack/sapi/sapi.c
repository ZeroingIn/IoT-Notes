    ...
    while ( pMsg )
    {
      switch ( pMsg->event )
      {
        ...
        case AF_INCOMING_MSG_CMD:
          ...
          /* user code start */
          uint8 Buf[3] = " ";
          uint8 information[5] = {0xFE,0x00,0x00,0x00,0x00};
          osal_memcpy(Buf,pMSGpkt->cmd.Data,3);          
          if(Buf[0] == 'W')
          {
            information[1] = 0x03;
            information[2] = 0x31;
            information[3] = Buf[1];
            information[4] = Buf[2];
            HalUARTWrite(HAL_UART_PORT_0, information,5);
          }
          if(Buf[0] == 'G')
          {
            information[1] = 0x02;
            information[2] = 0x30;
            information[3] = Buf[1];
            HalUARTWrite(HAL_UART_PORT_0, information,4);
          }
          if(Buf[0] == 'X')
          {
            if(Buf[1] == 0x01)  //判断上位机命令
            {
              set_relay(1);
              P1_3 = 1;
            }
