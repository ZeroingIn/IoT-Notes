...
void uartRxCB( uint8 port, uint8 event )
{
  ...
  if ( event != HAL_UART_TX_EMPTY ) 
  {
    ...
    if ( len>0 ) 
    {
      //HalUARTWrite(HAL_UART_PORT_0, pBuf, len); //测试成功   
      if(pBuf[0] == 0xFE && pBuf[1] == 0x02)
      {
        uint8 datas[2] = " ";
        datas[0] = 'X';
        datas[1] = pBuf[3]; //将继电器开关状态发送出去
        zb_SendZigbeeDatas(datas,2);
      }
...
void ChannelPanidInit (void)
{
  ...
  uint8 channel = 11;
  zb_Readpandid(panid);
  if(panid[0] != 0x87 || panid[1] != 0x80 || channel != zb_Readchannel())
  {
    panid[0] = 0x87;
    panid[1] = 0x80;
